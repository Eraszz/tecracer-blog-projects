locals {
  ...
  */
  ...
  /*
  The Kafka Consumer Account will need information regarding the cluster. The map 'kafka_cluster_information_map' contains all the
  necessary information
  */
  kafka_cluster_information_map = {
    for k, v in local.endpoint_service_key_map : k => {
      endpoint_url = local.broker_info_map[k].endpoint_url,
      broker_port  = local.broker_info_map[k].port,
